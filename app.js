...
window.GSATAppState = {
  getState: function(){
    ...
    return {
      ...
      report: {
        title: q('reportTitle')?.textContent || 'Governance Stress Test Report',
        currentText: q('reportCurrentText')?.textContent || q('stressText')?.textContent || '',
        targetText: q('reportTargetText')?.textContent || '',
        sheetGSAT: q('sheetGSAT')?.textContent || '',
        sheetGRR: q('sheetGRR')?.textContent || '',
        sheetTRM: q('sheetTRM')?.textContent || ''
      },
...
function openReportView(){
  try{
    const state = window.GSATAppState.getState();
    sessionStorage.setItem('hscs_gsat_report_state', JSON.stringify(state));
    window.location.href = 'report.html';
  }catch(err){
    console.error('Failed to open report view', err);
  }
}
})();
